...
BIP 77 is 3 round protocol between two cooperating parties, a sender and a receiver, which provides both a limited form of privacy improvements and blockspace savings opportunity.
...
- best effort broadcast
  - erasure coding
- gossip/epidemic broadcast - most efficient (constant number of connections per peer, not linear), least resillient
...
- brahms
- overlaybb
- https://dl.comp.nus.edu.sg/server/api/core/bitstreams/16a47ad4-bfbe-4146-bc30-916c9c691539/content

questions:
- open gossip (e.g. ownership proofs etc) vs. per session broadcast are qualitatively different
- can peers not involved in a session still be relied upon for driving session progress, or only for coalition formation related gossip?

# public infrastructure for assisting constrained clients

- allow some clients to opt out of being validators?
- payjoin directory service as semi-trusted party, providing a broadcast channel
...
- problem: bft consensus requires n >= 3f+1. dishonest majority or even just more than f malicious parties can disrupt liveness, denying service to honest parties
  - addressed to a reasonable extent by async fallback with BFT CRDTs and rate limiting total communication
  - with network partitions, DAPS and robust overlay network structure are best effort to allow honest parties on either side of a partition to 
...
- more diverse counterparties can improve privacy, but comes with risk of them being malicious
